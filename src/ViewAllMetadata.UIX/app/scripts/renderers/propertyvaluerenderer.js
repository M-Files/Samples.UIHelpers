function getLocaleDateString(forPlugin)
{
    const formats = {
        "af-ZA": "yyyy/MM/dd",
        "am-ET": "d/M/yyyy",
    ...
    renderer.getPropertyValue = function ()
    {
        ...
        switch (propertyDef.DataType)
        {
            case MFDatatypeDate:
                ...
                {
                    currentValue = dayjs(currentValue, getLocaleDateString(false)).utc().format("YYYY-MM-DD")
                }
                break;
            case MFDatatypeTimestamp:
                if ((currentValue + "").length > 0)
                {
                    currentValue = dayjs(currentValue, getLocaleDateString(false) + " HH:mm").utc().format("YYYY-MM-DD HH:mm:ss")
                }
                break;
        }
    ...
    function renderEditableValue($parent)
    {
        ...
        switch (propertyDef.DataType)
        {
            ...
            case MFDatatypeFloating:
                ...
                {
                    var format = getLocaleDateString(true);
                ...
                {
                    var format = getLocaleDateString(true) + " H:i";
                    $input.datetimepicker
                        (
                            {
                                datepicker: true,
